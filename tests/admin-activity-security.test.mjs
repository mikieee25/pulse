...
const migration = await readFile(new URL("../supabase/migrations/20260916060738_admin_activity_presence.sql", import.meta.url), "utf8")
const grantsMigration = await readFile(new URL("../supabase/migrations/20260916062851_restrict_admin_activity_table_grants.sql", import.meta.url), "utf8")
const adminPage = await readFile(new URL("../src/app/(dashboard)/admin/page.tsx", import.meta.url), "utf8")
const sidebar = await readFile(new URL("../src/components/layout/sidebar.tsx", import.meta.url), "utf8")
const activityComponent = await readFile(new URL("../src/components/admin/admin-activity.tsx", import.meta.url), "utf8")
...
test("activity and presence tables are protected for Admin reads", () => {
  ...
  assert.doesNotMatch(migration, /grant (all|insert|update|delete).*activity_log to authenticated/i)
  assert.match(grantsMigration, /revoke all on table public\.activity_log, public\.user_presence from authenticated, anon/i)
  assert.match(grantsMigration, /grant select on table public\.activity_log, public\.user_presence to authenticated/i)
})

test("Admin activity route and navigation are role protected", async () => {
  const page = await readFile(new URL("../src/app/(dashboard)/admin/activity/page.tsx", import.meta.url), "utf8").catch(() => "")
