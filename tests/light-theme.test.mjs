...
const theme = await readFile(new URL("../src/styles/theme.css", import.meta.url), "utf8").catch(() => "")
...
test("dashboard exposes a persistent light theme", () => {
  ...
  assert.doesNotMatch(theme, /linear-gradient\([^)]*doe-blue[^)]*doe-yellow|linear-gradient\([^)]*doe-yellow[^)]*doe-blue/i)
