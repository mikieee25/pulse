...
const categoryCostFormSource = await readFile(
...
const equipmentPageSource = await readFile(
  new URL("../src/app/(dashboard)/equipment/page.tsx", import.meta.url),
  "utf8"
);
...
const equipmentActionsSource = await readFile(
  new URL("../src/components/equipment/equipment-actions.tsx", import.meta.url),
  "utf8"
);
const categoryActionSource = await readFile(
  new URL("../src/app/actions/equipment-categories.ts", import.meta.url),
  "utf8"
);
const categoryMigrationSource = await readFile(
  new URL(
    "../supabase/migrations/20260915000000_normalize_equipment_categories.sql",
    import.meta.url
  ),
  "utf8"
);
const generalizedCategoryMigrationSource = await readFile(
  new URL(
    "../supabase/migrations/20260915000001_generalize_equipment_categories.sql",
    import.meta.url
  ),
  "utf8"
);
...
test("budget includes every database category, including manual replacement categories", () => {
  assert.doesNotMatch(
    budgetSource,
    /\.filter\(\s*\(category\) => category\.lifespan_years !== null\s*\)/
  );
  assert.match(`${budgetSource}\n${categoryCostFormSource}`, /lifespan_years/);
});
...
test("replacement plan seeds columns from the complete category list", () => {
  ...
  assert.match(
    dashboardSource,
    /allCategories = new Set\(categories\.map\(\(category\) => category\.name\)\)/
  );
});

test("PULSE uses its branded SVG instead of the default file favicon", async () => {
  await assert.rejects(
    access(new URL("../src/app/favicon.ico", import.meta.url))
  );
...
test("equipment exposes an admin-only circular category action", () => {
  ...
  assert.match(equipmentPageSource, /canManage &&[\s\S]*AddCategoryDialog/);
});
...
test("equipment category aliases share one reporting category", () => {
  ...
  assert.equal(
    canonicalEquipmentCategory("NOISE CANCELLING OVER-THE-HEAD HEADPHONES"),
    "Headphones"
  );
  assert.equal(
    canonicalEquipmentCategory("PRINTER (INKJET)"),
    "Printers & Scanners"
  );
  assert.equal(
    canonicalEquipmentCategory("DOCUMENT SCANNER"),
    "Printers & Scanners"
  );
  assert.equal(
    canonicalEquipmentCategory("WIRELESS MICROPHONE"),
    "Microphones"
  );
  assert.equal(
    canonicalEquipmentCategory("Wireless microphone"),
  ...
  assert.equal(
    canonicalEquipmentCategory("CAMERA MOUNTED SHOTGUN MIC"),
    "Microphones"
  );
  ...
  assert.equal(
    canonicalEquipmentCategory("USB 13-IN1 USB C HUB"),
    "USB Hubs & Splitters"
  );
  assert.equal(
    canonicalEquipmentCategory("SPLITTER 1 INPUT TO 4 OUTPUT 4K HDMI 2.0"),
    "USB Hubs & Splitters"
  );
  assert.equal(
    canonicalEquipmentCategory("WIRELESS KEYBOARD"),
    "Keyboards & Mice"
  );
  assert.equal(canonicalEquipmentCategory("TABLET PEN"), "Tablet Accessories");
  assert.equal(canonicalEquipmentCategory("SSD 1TB EXTERNAL"), "Storage");
  assert.equal(
    canonicalEquipmentCategory("PRESENTATION REMOTE CLICKER"),
    "Presentation Equipment"
  );
  assert.equal(
    canonicalEquipmentCategory("MOBILE PORTABLE WIFI HOTSPOT"),
    "Wi-Fi Hotspots"
  );
  ...
  assert.match(
    generalizedCategoryMigrationSource,
