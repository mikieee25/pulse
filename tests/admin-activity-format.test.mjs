...
test("formats equipment creation with actor and timestamp", () => {
  assert.match(
    formatActivityMessage({
      ...
      entityLabel: "Laptop Dell 5450",
      actorName: "Juan Dela Cruz",
      actorEmail: "juan@example.com",
      createdAt: timestamp,
    }),
    /Juan Dela Cruz .*added Laptop Dell 5450/i
  );
});
...
test("formats personnel updates and assignments", () => {
  assert.match(
    formatActivityMessage({
      ...
      entityLabel: "Maria Santos",
      actorName: "Admin",
      actorEmail: "admin@example.com",
      createdAt: timestamp,
    }),
    /Admin .*updated Maria Santos/i
  );
  assert.match(
    formatActivityMessage({
      action: "assigned",
      entityType: "equipment",
      entityLabel: "Camera Sony",
      actorName: "Admin",
      actorEmail: "admin@example.com",
      createdAt: timestamp,
    }),
    /assigned Camera Sony/i
  );
});
