...
test("temporary accounts are confirmed and marked for a forced password change", () => {
  assert.deepEqual(
    buildTemporaryUserAttributes("viewer@example.com", "LongEnough12!"),
    {
...
test("temporary passwords require at least 12 characters", () => {
  assert.equal(
    validateTemporaryPassword("Short12345!"),
...
test("new password and confirmation must match", () => {
  assert.equal(
    validatePasswordChange("LongEnough12!", "Different123!"),
...
test("flagged users are restricted to the password-change flow", () => {
  ...
  assert.equal(
    getPasswordChangeRedirect("/equipment", true),
    "/change-password"
  );
