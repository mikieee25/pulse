...
test("equipment status reflects lifecycle and manual condition signals", () => {
  ...
  assert.equal(
    equipmentDisplayStatus("Active", "Good", 2, 2025, today),
    "Expiring soon"
  );
  assert.equal(
    equipmentDisplayStatus("Active", "Good", null, 2020, today),
    "Active"
  );
  assert.equal(
    equipmentDisplayStatus("Active", "For Replacement", null, 2026, today),
    "For Replacement"
  );
  assert.equal(
...
test("replacement totals include lifecycle, manual replacement, and broken units", () => {
  ...
  assert.equal(
    needsReplacement("Active", "For Replacement", null, 2026, today),
    true
  );
...
test("active card includes operational replacement and expiry units, but not broken or retired units", () => {
  const stats = inventoryCardStats(
    [
      ...
      {
        status: "Active",
        condition_state: "For Replacement",
        lifespan_years: null,
        year_acquired: 2026,
      },
      {
