...
const { buildImportPlan, normalizeName } =
  await import("../scripts/import-list-json.ts");
const source = JSON.parse(
  await readFile(new URL("../list.json", import.meta.url), "utf8")
);
const importerSource = await readFile(
  new URL("../scripts/import-list-json.ts", import.meta.url),
  "utf8"
);
...
test("list import uses stable row identity instead of source row numbers", () => {
  ...
  const marker = original.rows[0].remarks.match(
    /\[Import:list\.json:[a-f0-9]{12}\]/
  )[0];
...
test("list import does not reuse row numbers from a replaced source file", () => {
  const plan = buildImportPlan(
    ...
    [],
    [{ id: "od-id", code: "OD" }],
    new Set(["[Import:list.json row 1]"])
  );

  assert.equal(plan.importableRows.length, 1);
  assert.match(
    plan.importableRows[0].remarks,
    /\[Import:list\.json:[a-f0-9]{12}\]/
  );
});
...
test("list import maps the two OD source division labels to OD", () => {
  ...
  assert.deepEqual(
    plan.rows.map((row) => row.division_id),
    ["od-id", "od-id"]
  );
});
...
test("list import resolves known personnel name variants", () => {
  const plan = buildImportPlan(
    [
      {
        Category: "GIMBAL",
        Description: "Test gimbal",
        "Serial No.": "TEST-1",
        Custodian: "Patrick T. Aquino",
        Assignee: "",
        Division: "OD",
      },
    ],
    [
      {
        id: "person-id",
        full_name: "Patrick T. Aquino, CESO III",
        division_id: "od-id",
