...
test("live notifications cover lifecycle and assignment signals", () => {
  const equipment = [
    {
      ...
      assignee_id: null,
      equipment_categories: { name: "Laptop", lifespan_years: 3 },
    },
    {
      id: "2",
      status: "Active",
      ...
      assignee_id: null,
      equipment_categories: { name: "Tablet", lifespan_years: 3 },
    },
    {
      id: "3",
      status: "Active",
      ...
      assigned_to: null,
      assignee_id: null,
      equipment_categories: { name: "Camera", lifespan_years: null },
    },
  ];
  ...
  assert.deepEqual(
    ...
    ["replacement", "expiring", "unassigned"]
  );
  ...
  assert.equal(
    first[0].id,
    buildNotifications(equipment, [], new Date("2026-09-11"))[0].id
  );
  ...
  assert.match(topbar, /getNotificationSnapshot/);
  assert.match(topbar, /buildNotificationsFromSnapshot/);
});
