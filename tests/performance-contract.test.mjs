...
const cacheSource = await readFile(
  new URL("../src/lib/cached-data.ts", import.meta.url),
  "utf8"
);
const tagsSource = await readFile(
  new URL("../src/lib/cache-tags.ts", import.meta.url),
  "utf8"
);
...
test("stable reference data uses tagged server caching", () => {
  ...
  assert.match(tagsSource, /pulse:inventory/);
  assert.match(tagsSource, /pulse:planning/);
  assert.match(cacheSource, /revalidate: 60/);
});

test("topbar reads the cached notification snapshot", () => {
  assert.match(topbarSource, /getNotificationSnapshot/);
...
test("activity and presence stay live instead of using stale cache", async () => {
  const activitySource = await readFile(
    new URL("../src/lib/admin-activity.ts", import.meta.url),
    "utf8"
  );
...
test("mutable reference data invalidates its cache tags", async () => {
  ...
  const personnelActions = await readFile(
