...
async function run() {
  ...
  const workbook = xlsx.readFile(
    process.env.PULSE_WORKBOOK_PATH || path.join(root, "01 EUMB ICT List.xlsx")
  );
  const dataRows = xlsx.utils.sheet_to_json<Row>(workbook.Sheets.Data, {
    defval: null,
  });
  ...
  const addDivision = (name: string, code = codeForDivision(name)) => {
    ...
    if (!divisions.has(normalizedCode))
      divisions.set(normalizedCode, {
        id: uuid(),
        code: normalizedCode,
        fullName: name,
      });
  ...
  const divisionCodeById = new Map(
    [...divisions.values()].map((division) => [division.id, division.code])
  );
  ...
  const personnel: Person[] = datasetRows.map((row) => {
    const match = dataRows.find(
      (excelRow) =>
        text(excelRow.Initials) === row.initials ||
        text(excelRow["Full Name"]).includes(row.fullName)
    );
    const rawPlantilla = text(match?.Plantilla);
    const plantillaStatus = validPlantilla.has(rawPlantilla)
      ? rawPlantilla
      : "For Transfer";
    if (!match) unmatchedPersonnel.push(`${row.fullName} (${row.initials})`);
    if (!validPlantilla.has(rawPlantilla))
      unresolvedPlantilla.push(`${row.fullName} (${rawPlantilla || "blank"})`);
  ...
  for (const person of personnel) {
    ...
    if (person.initials)
      personByName.set(person.initials.toLowerCase(), person);
  }
  ...
  for (const sheetName of equipmentSheets) {
    ...
    for (const row of rows) {
      ...
      if (!division)
        throw new Error(
          `Unknown equipment division ${divisionCode} in ${sheetName} row ${text(row["No."]) || "?"}`
        );
      const model = text(row.Model);
      const brand = text(row.Brand);
      const year = headerValue(row, [
        "Year Acquired",
        "Year Acquired/Transferred",
        "Year Acquired/ Transferred",
      ]);
      const custodian = text(row["Current Custodian"]);
      const lookup = (custodianOverrides[custodian] || custodian).toLowerCase();
      const match = custodian
        ? personByName.get(lookup) ||
          personnel.find(
            (person) =>
              person.fullName.toLowerCase().includes(lookup) ||
              lookup.includes(person.fullName.toLowerCase())
          )
        : undefined;
      ...
      const assignmentReasons = match
        ? [
            match.divisionId !== division.id
              ? `${divisionCode} equipment; ${divisionCodeById.get(match.divisionId) || "unknown"} personnel`
              : "",
            match.plantillaStatus !== "Regular"
              ? `personnel status ${match.plantillaStatus}`
              : "",
            ["PSS", "PES"].includes(match.position)
              ? `custodian position ${match.position} is not eligible`
              : "",
          ].filter(Boolean)
        : [];
      const assignmentIssue =
        match && assignmentReasons.length
          ? `${custodian} (${assignmentReasons.join("; ")})`
          : null;
      if (assignmentIssue)
        assignmentIssues.push(
          `${sheetName} #${text(row["No."]) || "?"} ${assignmentIssue}`
        );
      const blank = !model && !brand && !text(year);
      if (blank)
        blankRows.push(
          `${sheetName} #${text(row["No."]) || "?"} ${custodian || "unassigned"}`
        );
      const actualCategory =
        sheetName === "Laptop" && model.toLowerCase().includes("desktop")
          ? "Desktop"
          : sheetName;
      equipment.push({
        ...row,
  ...
  for (const division of divisions.values())
    seed += `INSERT INTO divisions (id, code, full_name) VALUES ('${division.id}', ${sql(division.code)}, ${sql(division.fullName)});\n`;
  for (const person of personnel)
    seed += `INSERT INTO personnel (id, full_name, initials, division_id, position, plantilla_status) VALUES ('${person.id}', ${sql(person.fullName)}, ${sql(person.initials)}, '${person.divisionId}', ${sql(person.position)}, ${sql(person.plantillaStatus)});\n`;
  ...
  for (const item of equipment) {
    ...
    const notes = [
      !text(item._model) && !text(item._brand) && !text(item._year)
        ? "[Migration] Needs data entry"
        : "",
      item.assignmentIssue
        ? `[Migration] Custodian division mismatch: ${item.assignmentIssue}; left unassigned`
        : "",
    ].filter(Boolean);
    const finalRemarks = [remarks, ...notes].filter(Boolean).join(" | ");
    const year = text(item._year)
      ? Number.parseInt(text(item._year), 10)
      : null;
    ...
    if (item.assignedTo)
      seed += `INSERT INTO assignment_history (equipment_id, personnel_id, note) VALUES ('${item.id}', '${item.assignedTo}', 'Imported from workbook');\n`;
  }
  ...
  const report = [
    ...
    "",
    "Sheet counts:",
    ...Object.entries(sheetCounts).map(
      ([name, count]) => `- ${name}: ${count}`
    ),
    "",
    "Unmatched personnel plantilla rows:",
    ...(unmatchedPersonnel.length
      ? unmatchedPersonnel.map((name) => `- ${name}`)
      : ["- None"]),
    "",
    "Unresolved plantilla values (stored as For Transfer):",
    ...(unresolvedPlantilla.length
      ? unresolvedPlantilla.map((name) => `- ${name}`)
      : ["- None"]),
    "",
    "Unmatched custodians:",
    ...(unmatchedCustodians.size
      ? [...unmatchedCustodians].map((name) => `- ${name}`)
      : ["- None"]),
    "",
    "Custodian assignment issues (left unassigned):",
    ...(assignmentIssues.length
      ? assignmentIssues.map((row) => `- ${row}`)
      : ["- None"]),
    "",
    "Blank data rows:",
