...
type Person = { id: string; full_name: string; division_id: string; position: string; plantilla_status: string }
...
function duplicateSerialGroups(rows: ImportRow[]) {
  ...
  for (const row of rows) {
    ...
    grouped.set(row.serial_number, [...(grouped.get(row.serial_number) || []), row])
  }
...
export function buildImportPlan(source: unknown[], people: Person[], divisions: Division[], existingMarkers = new Set<string>()) {
  ...
  for (const [index, raw] of source.entries()) {
    ...
    const division = custodian?.division_id || divisionsByCode.get(sourceDivision)?.id || null
    const custodianIsValid = Boolean(custodian && custodian.plantilla_status === "Regular" && !["PSS", "PES"].includes(custodian.position))
    const isSameAsCustodian = Boolean(assigneeName && normalizeName(assigneeName) === normalizeName(custodianName))
    const assigneeIsValid = Boolean(assignee && assignee.position && ["PSS", "PES"].includes(assignee.position) && assignee.division_id === division)
    const warnings: string[] = []

    if (!custodian) { unresolvedCustodians.add(custodianName); warnings.push("custodian not matched") }
    if (custodian && !custodianIsValid) warnings.push(`custodian ${custodian.plantilla_status} or position not eligible`)
    if (!divisionsByCode.has(sourceDivision) && !custodian) { unresolvedDivisions.add(sourceDivision); warnings.push("division not matched") }
...
async function fetchAll(supabase: SupabaseClient) {
  const [{ data: people, error: peopleError }, { data: divisions, error: divisionError }, { data: categories, error: categoryError }, { data: existing, error: equipmentError }] = await Promise.all([
    supabase.from("personnel").select("id,full_name,division_id,position,plantilla_status"),
  ...
  return { people: people as Person[], divisions: divisions as Division[], categories: categories as Array<{ id: string; name: string }>, existing: existing as ExistingEquipment[] }
}
...
async function run() {
  ...
  if (!Array.isArray(source)) throw new Error("list.json must contain an array of records.")
  ...
  if (categoryError) throw new Error(`Could not reload categories: ${categoryError.message}`)
  ...
  if (equipmentRows.some((row) => !row.category_id || !row.division_id)) throw new Error("Import plan contains an unresolved category or division.")
  const { error: equipmentError } = await supabase.from("equipment").insert(equipmentRows)
  if (equipmentError) throw new Error(`Could not import equipment: ${equipmentError.message}`)

  const historyRows = equipmentRows.flatMap((row) => row.assigned_to ? [{ equipment_id: row.id, personnel_id: row.assigned_to, assignment_type: "Custodian", note: "Imported from list.json" }] : [])
  if (historyRows.length) {
    const { error: historyError } = await supabase.from("assignment_history").insert(historyRows)
    if (historyError) {
      await supabase.from("equipment").delete().in("id", equipmentIds)
      throw new Error(`Assignment history failed; imported equipment was rolled back: ${historyError.message}`)
    }
  }

  const { data: verified, error: verifyError } = await supabase.from("equipment").select("id").in("id", equipmentIds)
  if (verifyError || verified?.length !== equipmentIds.length) throw new Error(`Import verification failed: expected ${equipmentIds.length}, found ${verified?.length || 0}.`)
  console.log(`Imported ${equipmentIds.length} equipment records and ${newCategories.length} new categories.`)
}
