...
export function UserManagement({
  ...
  return (
    <div className="space-y-6">
      <SectionPanel
        title="Add user"
        description="Create a login with a temporary password (min 12 chars). You must provide one below."
      >
        <form
          onSubmit={addUser}
          className="grid gap-3 p-5 sm:grid-cols-2 xl:grid-cols-6"
        >
          <Input
            required
            className="h-9"
      ...
      {successMsg && (
        <p
          role="status"
          className="rounded-xl border border-pulse/20 bg-pulse/5 px-4 py-3 text-sm text-pulse"
        >
          {successMsg}
        </p>
      )}
      ...
      >
        <div className="overflow-auto">
          <table className="w-full min-w-[760px] text-sm">
            ...
            <thead className="bg-canvas text-left text-slate">
            ...
            <tbody>
              ...
              {!users.length && (
                <tr>
                  <td colSpan={3} className="h-32 text-center text-slate">
                    <UsersRound
