  ...
  return <div className="flex flex-col xl:flex-row gap-8 items-start justify-between">
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 items-center">
        <select disabled={pendingAction !== null} aria-label="Select custodian" value={selectedCustodian} onChange={(event) => setSelectedCustodian(event.target.value)} className="h-8 w-56 rounded-lg border border-line bg-canvas px-2.5 text-sm text-paper font-sans outline-none focus-visible:ring-2 focus-visible:ring-pulse/50 transition-all">
        ...
        <Button variant="outline" disabled={pendingAction !== null} onClick={saveCustodian}>{pendingAction === "custodian" ? "Saving…" : "Save Custodian"}</Button>
      </div>
      <div className="flex flex-wrap gap-2 items-center">
        <select disabled={pendingAction !== null} aria-label="Select assignee" value={selectedAssignee} onChange={(event) => setSelectedAssignee(event.target.value)} className="h-8 w-56 rounded-lg border border-line bg-canvas px-2.5 text-sm text-paper font-sans outline-none focus-visible:ring-2 focus-visible:ring-pulse/50 transition-all">
        ...
        <Button variant="outline" disabled={pendingAction !== null} onClick={saveAssignee}>{pendingAction === "assignee" ? "Saving…" : "Save Assignee"}</Button>
      </div>
      <div className="flex flex-wrap gap-2 items-center pt-2 border-t border-line">
        <Button variant="outline" disabled={pendingAction !== null} onClick={retire}>{pendingAction === "archive" ? "Archiving…" : "Archive"}</Button>
      </div>
      {message && <p role={messageIsError ? "alert" : "status"} className="text-sm text-slate">{message}</p>}
    </div>
      ...
      <div className="flex items-center rounded-lg border border-line bg-canvas p-1">
         {["Good", "For Replacement", "Broken"].map(s => {
            ...
            const activeColor = s === "Good" ? "text-pulse" : s === "For Replacement" ? "text-alert" : "text-warning"
             return <button key={s} type="button" disabled={pendingAction !== null} aria-pressed={isActive} aria-labelledby="equipment-state-label" onClick={() => handleStateChange(s as EquipmentCondition)} className={`flex-1 px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${isActive ? `${activeColor} bg-canvas-deep shadow-sm` : 'text-slate hover:text-paper'}`}>
