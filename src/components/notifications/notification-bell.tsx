...
function readStoredIds(userKey: string) {
  try {
    const stored = JSON.parse(
      window.localStorage.getItem(`${STORAGE_KEY}:${userKey}`) || "[]"
    );
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div
          id={MENU_ID}
          className="absolute right-0 top-11 z-50 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-lg border border-line bg-canvas-deep shadow-2xl shadow-black/30"
        >
            ...
            <div className="divide-y divide-line/70">
              {notifications
                ...
                .map((notification) => (
                  ...
                  >
                    <span
                      className={`mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full ${toneClass(notification.tone)}`}
                    >
