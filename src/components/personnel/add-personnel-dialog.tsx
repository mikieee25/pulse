...
export function AddPersonnelDialog({
  ...
  children: ReactNode;
  divisions: Division[];
  initial?: PersonnelFormValue;
}) {
  const [open, setOpen] = useState(false);
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState<PersonnelInput>(
    initial || {
      full_name: "",
      initials: "",
      position: "",
  ...
  const set = <K extends keyof PersonnelInput>(
    key: K,
    value: PersonnelInput[K]
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = initial?.id
        ? await updatePersonnel(initial.id, {
            ...form,
            initials: form.initials || suggestedInitials(form.full_name),
          })
        : await addPersonnel({
            ...form,
            initials: form.initials || suggestedInitials(form.full_name),
      ...
      if (result.error) setError(result.error);
        ...
        setOpen(false);
        if (!initial?.id)
          setForm({ ...form, full_name: "", initials: "", position: "" });
      }
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[520px] bg-canvas-deep border-line text-paper">
        <DialogHeader>
          <DialogTitle>
            {initial?.id ? "Edit personnel" : "Add personnel"}
          </DialogTitle>
          <DialogDescription className="text-slate">
            Initials are suggested from the name but remain editable.
          </DialogDescription>
        </DialogHeader>
            ...
            <Input
              ...
              onChange={(e) =>
                setForm((current) => ({
                  ...
                  initials:
                    current.initials || suggestedInitials(e.target.value),
                }))
              }
            />
          </label>
