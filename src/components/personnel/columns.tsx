...
export const getPersonnelColumns = (
  divisions: Division[],
  canManage: boolean
): ColumnDef<PersonnelData>[] => {
  const baseColumns: ColumnDef<PersonnelData>[] = [
    { accessorKey: "full_name", header: "Full Name" },
    {
      accessorKey: "position",
      header: "Position",
      cell: ({ row }) => (
        <span className="text-slate">{row.original.position || "-"}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-slate">
          {row.original.equipment_count !== undefined
            ? `${row.original.equipment_count} ${row.original.equipment_count === 1 ? "unit" : "units"}`
            : row.original.equipment
                ?.map((item) =>
                  `${item.brand || ""} ${item.model || "unit"}`.trim()
                )
                .join(", ") || "None"}
        </span>
      ),
  ...
  return canManage
    ? [
        ...baseColumns,
        {
          ...
          header: "Actions",
          cell: ({ row }) => (
            <PersonnelActions person={row.original} divisions={divisions} />
          ),
        },
      ]
    : baseColumns;
