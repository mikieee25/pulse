...
export async function POST(request: Request) {
  ...
  if (!admin)
    return changePasswordError(
      request,
      "Password change configuration is incomplete."
    );
  ...
  if (metadataError)
    return changePasswordError(
      request,
      "Password changed, but your account could not be unlocked. Try again."
    );

  await supabase.auth.signOut();
  return NextResponse.redirect(
    new URL(
      "/login?message=Password changed. Sign in with your new password.",
      request.url
    ),
    303
  );
