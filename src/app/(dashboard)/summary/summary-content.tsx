...
export function SummaryContent({
  ...
  const grandTotal = initialData.reduce(
    (sum, item) => sum + item.rate * item.units,
    0
  );
  const totalUnits = initialData.reduce((sum, item) => sum + item.units, 0);
  ...
  return (
    <div className="space-y-8 pb-8">
      ...
      >
        <h2 id="summary-overview-title" className="sr-only">
          Summary overview
        </h2>
        <MetricCard
          label="Grand total budget"
          value={formatMoney(grandTotal)}
          detail={`FY ${viewYear} forecast`}
          icon={Banknote}
          tone="pulse"
        />
        <MetricCard
          ...
          value={totalUnits}
          detail={`${divisions.length} requesting divisions`}
          icon={PackageCheck}
          tone="warning"
        />
      </section>
      ...
      <SectionPanel
        ...
        description="Requested units and forecast totals by equipment category"
        actions={
        ...
        <div className="overflow-auto">
          <table className="w-full min-w-[760px] text-sm">
            <caption className="sr-only">
              Division summary for FY {viewYear}
            </caption>
              ...
              <tr className="border-b border-line text-left text-slate">
                ...
                {categories.map((category) => (
                  <th
                    key={category}
                    scope="col"
                    className="border-l border-line px-4 py-3 text-center font-medium"
                  >
                    <div>{category}</div>
                    <div className="text-[11px] font-normal text-slate">
                      Rate: {formatMoney(categoryRates[category] || 0)}
                    </div>
                  </th>
                ))}
                <th
                  scope="col"
                  className="border-l border-line px-5 py-3 text-right font-medium text-pulse"
                >
                  Division total
                </th>
              </tr>
            ...
            <tbody>
              {divisions.slice(0, pageSize).map((div) => {
                const divItems = initialData.filter(
                  (d) => d.division?.code === div
                );
                ...
                return (
                  ...
                  >
                    ...
                    {categories.map((category) => {
                      const count = divItems
                        .filter((item) => item.catName === category)
                        .reduce((sum, item) => sum + item.units, 0);
                      return (
                        <td
                          key={category}
                          className={`border-l border-line px-4 py-3 text-center tabular-nums ${count ? "text-pulse" : "text-slate/50"}`}
                        >
                          {count || "—"}
                        </td>
                      );
                    })}
                    <td className="border-l border-line px-5 py-3 text-right font-semibold tabular-nums text-paper">
                      {formatMoney(divTotal)}
                    </td>
                  </tr>
                );
              })}
              <tr className="border-t-2 border-line bg-canvas text-paper">
                ...
                {categories.map((category) => (
                  <td
                    key={category}
                    className="border-l border-line px-4 py-4 text-center font-semibold tabular-nums text-pulse"
                  >
                    {initialData
                      .filter((item) => item.catName === category)
                      .reduce((sum, item) => sum + item.units, 0) || "—"}
                ...
                <td className="border-l border-line px-5 py-4 text-right text-base font-bold tabular-nums text-pulse">
