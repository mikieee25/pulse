...
export default async function EquipmentDetailPage({
  ...
  const [profile, equipmentResult, categoriesResult] = await Promise.all([
    ...
    supabase
      .from("equipment")
      .select(
        "*,division:divisions(full_name,code),personnel!equipment_assigned_to_fkey(full_name,position,plantilla_status),assignee:personnel!equipment_assignee_id_fkey(full_name,position,plantilla_status),equipment_categories(name,lifespan_years)"
      )
      .eq("id", id)
      .single(),
    getCachedCategories(),
  ...
  if (equipmentError && equipmentError.code !== "PGRST116") {
    console.error("Equipment detail query failed", {
    ...
    return (
      <div className="rounded-lg border border-alert/30 bg-alert/10 p-6 text-alert">
        Equipment details are unavailable. Try refreshing.
      </div>
    );
  }
  ...
  const [
    ...
    supabase
      ...
      .select(
        "id,assigned_at,unassigned_at,note,assignment_type,personnel(full_name)"
      )
      .eq("equipment_id", id)
  ...
  if (personnelError || historyError || categoriesError) {
    ...
    console.error("Equipment detail support query failed", {
      message:
        typeof queryError === "string" ? queryError : queryError?.message,
    });
    return (
      <div className="rounded-lg border border-alert/30 bg-alert/10 p-6 text-alert">
        Equipment assignment data is unavailable. Try refreshing.
      </div>
    );
  }
  ...
  const division = equipment.division
    ? [
        {
          id: equipment.division_id,
          code: equipment.division.code,
          full_name: equipment.division.full_name,
        },
      ]
    : [];
  const categoryOptions = Array.from(
    new Set(
      (categories || []).map((category) =>
        canonicalEquipmentCategory(category.name)
      )
    )
  ).filter(Boolean);
    ...
    <div className="space-y-8 pb-8">
          ...
          <>
            ...
            {canManage && (
              <AddEquipmentDialog
                ...
                categories={
                  categoryOptions.length ? categoryOptions : [categoryName]
                }
                divisions={division}
                ...
                initial={{
                  ...
                  year_acquired: equipment.year_acquired,
                  serial_number: equipment.serial_number,
                  procurement_method: equipment.procurement_method,
                  division_id: equipment.division_id,
                  assigned_to: equipment.assigned_to,
                  assignee_id: equipment.assignee_id,
                  condition_state:
                    equipment.condition_state as EquipmentInput["condition_state"],
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <SectionPanel
          ...
          description="Core identity and procurement details"
        >
          <dl className="grid grid-cols-2 gap-x-6 gap-y-5 p-5">
            <div>
              <dt className="text-xs text-slate">Category</dt>
              <dd className="mt-1 text-sm text-paper">{categoryName}</dd>
            </div>
            <div>
            ...
            <div className="col-span-2">
              <dt className="text-xs text-slate">Procurement method</dt>
              <dd className="mt-1 text-sm text-paper">
                {equipment.procurement_method || "N/A"}
              </dd>
            </div>
          </dl>
        </SectionPanel>
        ...
        <SectionPanel
          ...
          description="Current custodian, assignee, and division"
        >
          <dl className="grid grid-cols-2 gap-x-6 gap-y-5 p-5">
            <div>
              <dt className="text-xs text-slate">Custodian</dt>
              <dd className="mt-1 text-sm text-paper">
                {equipment.personnel?.full_name || "Unassigned"}
              </dd>
            </div>
            <div>
              <dt className="text-xs text-slate">Position</dt>
              <dd className="mt-1 text-sm text-paper">
                {equipment.personnel?.position || "-"}
              </dd>
            </div>
            <div>
              <dt className="text-xs text-slate">Assignee</dt>
              <dd className="mt-1 text-sm text-paper">
                {equipment.assignee?.full_name || "Unassigned"}
              </dd>
            </div>
            <div>
              <dt className="text-xs text-slate">Position</dt>
              <dd className="mt-1 text-sm text-paper">
                {equipment.assignee?.position || "-"}
              </dd>
            </div>
            <div className="col-span-2">
              <dt className="text-xs text-slate">Division</dt>
              <dd className="mt-1 text-sm text-paper">
                {equipment.division?.full_name || "-"}
              </dd>
            </div>
          </dl>
        </SectionPanel>
      ...
      {canManage && (
        <SectionPanel
          ...
          description="Update custodian, assignee, and equipment condition"
        >
          <div className="p-5">
            <EquipmentActions
              id={equipment.id}
      ...
      <SectionPanel
        ...
        description="Recorded assignment events for this asset"
      >
        {history.length ? (
              ...
              <li key={entry.id} className="border-l-2 border-pulse pl-4">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="text-sm font-medium text-paper">
                    {entry.personnel?.full_name || "Unknown personnel"}
                  </p>
                ...
                <p className="mt-1 text-xs text-slate">
                  Assigned {new Date(entry.assigned_at).toLocaleDateString()}
                  {entry.unassigned_at
                    ? ` · ended ${new Date(entry.unassigned_at).toLocaleDateString()}`
                    : " · current"}
                </p>
                {entry.note && (
                  <p className="mt-1 text-sm text-slate">{entry.note}</p>
                )}
              </li>
            ))}
          </ol>
          ...
          <EmptyState
            ...
            description="Assignment events will appear here when this asset is updated."
          />
        )}
      </SectionPanel>
      ...
      {equipment.remarks && (
        <SectionPanel
          title="Remarks"
          description="Additional notes recorded for this asset"
        >
          <div className="flex gap-3 p-5 text-sm text-slate">
            <ClipboardList
