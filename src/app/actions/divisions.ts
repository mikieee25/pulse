...
export async function addDivision(input: DivisionInput) {
  ...
  const { data: saved, error } = await supabase.from("divisions").insert({ code: parsed.data.code.toUpperCase(), full_name: parsed.data.full_name }).select("id").single()
  if (error) return { error: error.message }
  await recordActivity({ action: "created", entityType: "division", entityId: saved.id, entityLabel: parsed.data.code.toUpperCase(), divisionName: parsed.data.full_name })
...
export async function updateDivision(id: string, input: DivisionInput) {
  const access = await requireProfile("Admin")
  if (access.error) return access
  if (!divisionId.safeParse(id).success) return { error: "Division not found." }
  const parsed = divisionInput.safeParse(input)
  if (!parsed.success) return { error: "Please complete the division fields." }
  const supabase = await createClient()
  ...
  await recordActivity({ action: "updated", entityType: "division", entityId: id, entityLabel: parsed.data.code.toUpperCase(), divisionName: parsed.data.full_name })
  revalidateTag(PULSE_CACHE_TAGS.divisions, "max")
  revalidatePath("/divisions")
  return { success: true }
}
