...
const userInput = z.object({
  ...
  division_scope: z.string().uuid().nullable(),
});
...
export async function createUser(input: z.infer<typeof newUserInput>) {
  ...
  const { data, error } = await admin.auth.admin.createUser(
    buildTemporaryUserAttributes(profile.email, temporary_password)
  );
  if (error || !data.user)
  ...
  if (profileError) {
    ...
    return {
      error: rollbackError
        ? `${profileError.message} Cleanup also failed; remove the Auth user manually.`
...
export async function updateUser(
  id: string,
  input: Pick<z.infer<typeof userInput>, "role" | "division_scope">
) {
  const access = await requireProfile("Admin");
  if (access.error) return access;
  ...
  if (id === access.profile.id && parsed.data.role !== "Admin")
    return { error: "You cannot remove your own administrator access." };
  const { data: target, error: targetError } = await supabase
    .from("app_users")
    .select("id,role,full_name,email,division_scope")
    .eq("id", id)
    .single();
