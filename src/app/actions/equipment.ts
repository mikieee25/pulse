...
const equipmentInput = z.object({
  ...
  condition_state: z
    .enum(["Good", "For Replacement", "Broken"])
    .default("Good"),
...
export async function addEquipment(input: EquipmentInput) {
  ...
  if (!parsed.success)
    return { error: "Please complete the equipment fields." };
  const supabase = await createClient();
  const custodianError = await validateAssignment(
    supabase,
  ...
  const assigneeError = await validateAssignment(
    supabase,
    parsed.data.division_id,
...
export async function reassignEquipment(
  ...
  personnelId: string | null,
  note = "",
  role: "Custodian" | "Assignee" = "Custodian"
) {
  const access = await requireProfile("Admin");
  if (access.error) return access;
  if (
    !equipmentId.safeParse(id).success ||
    (personnelId !== null && !equipmentId.safeParse(personnelId).success)
  )
    return { error: "Invalid equipment assignment." };
  const supabase = await createClient();
  ...
  const assignmentError = await validateAssignment(
    supabase,
    equipment.division_id,
    personnelId,
    role
  );
  if (assignmentError) return { error: assignmentError };

  const { error } = await supabase.rpc("reassign_equipment", {
    p_equipment_id: id,
