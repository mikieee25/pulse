...
export function parseEquipmentFilters(input: SearchInput): EquipmentFilters {
  ...
  return {
    ...
    status: (EQUIPMENT_FILTER_STATUSES as readonly string[]).includes(status) ? status as EquipmentFilterStatus : "",
    assignment: textValue(input, "assignment") === "unassigned" ? "unassigned" : "",
    page: Number.isInteger(page) && page > 0 ? page : 1,
  }
}

export function equipmentFiltersQuery(filters: EquipmentFilters, category?: string) {
